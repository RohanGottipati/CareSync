  ...
  const features = [
    {
      ...
      desc: "A high-level command view of all clinical flags across your entire roster. Our sentinel runs continuously, surfacing medication risks and behavioral anomalies instantly — so you act before incidents occur.",
    },
    {
      icon: <Users2 size={22} />,
      color: "bg-indigo-100 text-indigo-600",
      title: "Role & Access Management",
      desc: "Absolute control over who sees what. Assign staff and family members to specific client threads with clinical-level security. Data flows only where it's explicitly authorized — no exceptions.",
    },
    {
      icon: <ShieldCheck size={22} />,
      color: "bg-emerald-100 text-emerald-600",
      title: "Audit & Compliance Log",
      desc: "Every visit, every medication log, every flag resolution — timestamped and immutable. Our compliance engine keeps you audit-ready at all times, generating reports in seconds, not hours.",
    },
    {
      icon: <Zap size={22} />,
      color: "bg-amber-100 text-amber-600",
      title: "Instant Escalation Engine",
      desc: "When something looks wrong, coordinators are the first to know. Automated escalation pathways mean critical flags go directly to the right person — no missed calls, no delayed responses.",
    },
  ];

  const steps = [
    { num: "01", text: "Manage Roster & Assignments" },
    { num: "02", text: "Morning Sentinel Audit" },
    { num: "03", text: "Audit Clinical Flags" },
    { num: "04", text: "Coordinate Communications" },
  ];
  ...
  return (
    <div className="bg-white" ref={heroRef}>
      ...
      <svg className="absolute -top-[9999px] -left-[9999px] w-0 h-0 pointer-events-none">
        <defs>
          <clipPath id="clip-coord-1" clipPathUnits="objectBoundingBox">
          ...
          <clipPath id="clip-coord-2" clipPathUnits="objectBoundingBox">
          ...
          <clipPath id="clip-coord-3" clipPathUnits="objectBoundingBox">
          ...
          <clipPath id="clip-coord-4" clipPathUnits="objectBoundingBox">
      ...
      <section className="relative py-28 px-4 bg-slate-50 overflow-hidden">
        <TimelineContent
          ...
          timelineRef={heroRef}
        />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_40%_at_50%_80%,rgba(99,102,241,0.08),transparent)] pointer-events-none" />
        ...
        <div className="relative z-10 max-w-2xl mx-auto text-center">
          <TimelineContent
            ...
            customVariants={fadeIn}
            timelineRef={heroRef}
            className="inline-flex items-center gap-2 bg-indigo-50 border border-indigo-200 text-indigo-700 text-xs font-bold uppercase tracking-widest px-4 py-2 rounded-full mb-8"
          >
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-black text-gray-900 mb-6 leading-[1.1] tracking-tight">
            ...
            <VerticalCutReveal
              ...
              containerClassName="justify-center text-center text-indigo-600"
            >
              Total Control.
            </VerticalCutReveal>
          </h1>
          ...
          >
            WardRound gives coordinators a unified command layer — real-time clinical visibility,
            zero-risk oversight, automated compliance, and instant escalation across your entire roster.
          </TimelineContent>
          ...
          >
            <button
              ...
              className="bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-600/30 flex items-center gap-2 hover:gap-4 transition-all duration-300 text-white px-7 py-3.5 rounded-full font-semibold text-base cursor-pointer"
            >
              Open Admin Suite <ArrowRight size={18} />
            </button>
            <button className="bg-white border border-gray-200 hover:border-indigo-300 hover:bg-indigo-50 text-gray-700 px-7 py-3.5 rounded-full font-semibold text-base transition-all duration-300 cursor-pointer">
              Learn More
            </button>
          </TimelineContent>
        </div>
      ...
      <section className="py-24 px-4 bg-slate-50">
        <div className="max-w-5xl mx-auto">
          <TimelineContent as="div" animationNum={0} customVariants={revealUp} timelineRef={heroRef} className="text-center mb-16">
            <span className="text-indigo-600 font-bold text-sm uppercase tracking-widest">Command Tools</span>
            ...
            <p className="text-gray-500 max-w-xl mx-auto">Built for coordinators who need total oversight without drowning in noise — intelligent filtering, instant action.</p>
          </TimelineContent>
      ...
      <section className="py-24 px-4 bg-white">
        <div className="max-w-5xl mx-auto grid md:grid-cols-2 gap-16 items-center">
          <TimelineContent as="div" animationNum={0} customVariants={revealUp} timelineRef={heroRef}>
            <span className="text-indigo-600 font-bold text-sm uppercase tracking-widest">Workflow Optimization</span>
            ...
            <p className="text-gray-500 leading-relaxed mb-8">
              A structured daily protocol that ensures no client ever falls through the cracks — from morning audit to end-of-day compliance check.
            </p>
            <button
              onClick={() => navigate("/coordinator")}
              className="bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-600/25 flex items-center gap-2 hover:gap-4 transition-all duration-300 text-white px-7 py-3.5 rounded-full font-semibold text-base cursor-pointer"
            >
              Open Admin Suite <ArrowRight size={18} />
            </button>
          </TimelineContent>
          ...
          <div className="space-y-4">
            {steps.map((step, i) => (
              <TimelineContent
                ...
                customVariants={revealUp}
                timelineRef={heroRef}
                className="flex items-center gap-5 bg-slate-50 rounded-2xl px-6 py-5 border border-gray-100 hover:border-indigo-200 hover:bg-indigo-50/50 transition-all duration-300 group"
              >
                <span className="w-10 h-10 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xs font-black flex-shrink-0 group-hover:scale-110 transition-transform">
                  {step.num}
                </span>
                <span className="text-gray-800 font-semibold">{step.text}</span>
                <CheckCircle2 size={18} className="text-indigo-400 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" />
              </TimelineContent>
            ))}
          </div>
        </div>
      ...
      {/* ─── CTA ─── */}
      <section className="py-20 px-4 bg-gradient-to-br from-slate-800 to-indigo-900">
        <TimelineContent as="div" animationNum={0} customVariants={fadeIn} timelineRef={heroRef} className="max-w-2xl mx-auto text-center">
          ...
          <p className="text-slate-300 mb-8 text-lg">
            WardRound gives coordinators the visibility and control they need to deliver exceptional care at scale.
          </p>
          <button
            onClick={() => navigate("/coordinator")}
            className="bg-white hover:bg-indigo-50 text-indigo-700 font-bold px-8 py-4 rounded-full shadow-xl flex items-center gap-2 mx-auto hover:gap-4 transition-all duration-300 cursor-pointer"
          >
            Access Admin Suite <ArrowRight size={18} />
          </button>
        </TimelineContent>
      </section>
    </div>
